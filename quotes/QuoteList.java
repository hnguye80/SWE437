...
public class QuoteList
{
	...
	/* modified */ static final int SearchKeywordVal   = 3;
	...
	private boolean isDuplicateQuote(Quote q) {
		boolean isDuplicate = false;
		Quote currentQuote = new Quote();

		for (int i = 0; i < quoteArray.size(); i++) {
			currentQuote = quoteArray.get(i);
			if ((currentQuote.getAuthor().toLowerCase().indexOf (q.getAuthor().toLowerCase()) != -1 ||
					currentQuote.getQuoteText().toLowerCase().indexOf (q.getQuoteText().toLowerCase()) != -1)) {
				isDuplicate = true;
			}
		...
		if (!searchString.isEmpty()) {
			for (int i = 0; i < quoteArray.size(); i++)
			{
				...
				} else if (mode == SearchKeywordVal)        	  
				{
					int keywordIndex;
					try {
						// split the keyword into a list
						String[] keywordList = searchString.toLowerCase().split(",", -1);
						// loop through each element in the keywordList and determine
						// whether the current quote matches the keyword
						for(keywordIndex = 0; keywordIndex < keywordList.length; keywordIndex++) {
							
							if(!quote.getKeyword().isEmpty() && !quote.getKeyword().equals(null) && 
								(quote.getKeyword().toLowerCase().indexOf (keywordList[keywordIndex].trim()) != -1) )
							{
